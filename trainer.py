...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False, dataset='IEMOCAP', loss_type=''):
    losses, preds, labels = [], [], []
    losses_a, preds_a = [], []
    losses_v, preds_v = [], []
    losses_l, preds_l = [], []
    ...
    for data in dataloader:
        ...
        if len(modals)==3: 
            prob_a, prob_v, prob_l, prob_share, prob = model(textf, qmask, umask, lengths, max_seq_length, acouf, visuf)
        if len(modals)==2:
            prob_0, prob_1, prob_share, prob = model(textf, qmask, umask, lengths, max_seq_length, acouf, visuf)
        if len(modals)==1:
        ...
        if len(modals)==3:
            ...
            loss_share = loss_function(prob_share, label)
        if len(modals)==2:
        ...
        if loss_type=='auto_multi_loss':
            if len(modals)==3:
                awl = AutomaticWeightedLoss(5)
                loss = awl(loss_a, loss_v, loss_l, loss_share, loss)
            if len(modals)==2:
                awl = AutomaticWeightedLoss(4)
                loss = awl(loss_0, loss_1, loss_share, loss)
            if len(modals)==1:
                awl = AutomaticWeightedLoss(2)
                loss = awl(loss_0, loss)
        elif loss_type=='manual_multi_loss':
            loss = loss_a + loss_v + loss_l + loss_share + loss
        elif loss_type=='single_loss':
            loss = loss
        else:
            NotImplementedError
        ...
        if train:
            loss.backward()
            if args.GE:
                for names, parms in model.named_parameters():
