...
import time
from utils import AutomaticWeightedLoss, Focal_Loss, Poly1_Focal_Loss
...
save_path = './save_model/best_model_{}/model_{}_{}{}{}_{}{}{}_{}_{}{}.pth'.format(
    ...
    args.nheads, args.fusion_method, args.agg_type)
...
def main(rank, world_size):
    ...
    if args.loss_type=='auto_multi_loss':
        if len(modals)==3:
    ...
    for e in range(n_epochs):
        ...
        train_loss, train_acc, _, _, train_fscore, _, _, _, _, _ , train_loss_a, train_acc_a, train_fscore_a, train_loss_v, train_acc_v, train_fscore_v, train_loss_l, train_acc_l, train_fscore_l = train_or_eval_graph_model(model, loss_function, train_loader, e, cuda, args.modals, optimizer, True, dataset=args.Dataset, loss_type=args.loss_type)
        valid_loss, valid_acc, _, _, valid_fscore, _, _, _, _, _, valid_loss_a, valid_acc_a, valid_fscore_a, valid_loss_v, valid_acc_v, valid_fscore_v, valid_loss_l, valid_acc_l, valid_fscore_l = train_or_eval_graph_model(model, loss_function, valid_loader, e, cuda, args.modals, dataset=args.Dataset, loss_type=args.loss_type)
        ...
        if rank == 0:
            test_loss, test_acc, test_label, test_pred, test_fscore, _, _, _, _, _ , test_loss_a, test_acc_a, test_fscore_a, test_loss_v, test_acc_v, test_fscore_v, test_loss_l, test_acc_l, test_fscore_l = train_or_eval_graph_model(model, loss_function, test_loader, e, cuda, args.modals, dataset=args.Dataset, loss_type=args.loss_type)
